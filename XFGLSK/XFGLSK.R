...
# Book:          XFG
# -----------------------------------------------------------------------
# See also:      XFGiv00, XFGiv01, XFGiv02, XFGiv03, XFGiv04, XFGiv05,
#                XFGiv06
# -----------------------------------------------------------------------
# Quantlet:      XFGLSK
# -----------------------------------------------------------------------
# Description:   XFGLSK performes a Least Squares Kernel Smoothing technique
#                for implied volatility smile estimation. To achieve good 
#                forecast of asset price volatility the Gaussian shaped vega
#                function is used as weight. A quartic kernel is employed.
#                Given global convexity, the Golden section search is
#                implemented with estimation tolerance 10e-5.
#                The data consists of January and February 2001 tick 
#                statistics of the DAX futures contracts and DAX index options.
#                Since the data are transaction data containing potential 
#                misprints, a filter in deleting all observations whose 
#                implied volatility is bigger than 0.7 and less than 0.1
#                is applied.
#                Plots of observed option price data are presented. From
#                the lower left to upper right put prices are displayed,
#                from upper left to lower right (normalized) call prices.
#                Moreover, graphs of least squares kernel smoothed implied
#                volatility smile for 17 days to expiry (January 02, 2001)
#                with bandwidth = 0.025 and 95 percent confidence bands
#                and 14 days to expiry (February 02, 2001) with bandwidth
#                = 0.015 and 95 percent confidence bands are shown.
#                Regression smoothing is made employing a Nadaraya Watson
#                kernel regression estimate for quartic kernel
# -----------------------------------------------------------------------
# Keywords:      least squares, LSKE, kernel smoothing, implied volatility,
#                nonparametric, KDE, Golden section, vega, Gaussian,
#                forecast, option, bandwidth, confidence bands,
#                Nadaraya Watson
# -----------------------------------------------------------------------
# Usage:         -
# -----------------------------------------------------------------------
# Author:        Awdesch Melzer 20140605
# -----------------------------------------------------------------------
...
kern= function(x,f){
	x = as.matrix(x)
	if(f == "gau"){ # gaussian kernel
		y = dnorm(x)
	}else if(f == "qua"){ # quadric / biweight kernel
		y = 0.9375*(abs(x)<1)*(1-x^2)^2
	}else if(f == "cosi"){ # cosine kernel
		y = (pi/4)*cos((pi/2)*x)*(abs(x)<1)
	}else if(f == "tri"){ # triweight kernel
		y = 35/32*(abs(x)<1)*(1-x^2)^3
	}else if(f == "tria"){ # triangular kernel
		y = (1 - abs(x))*(abs(x)<1)
	}else if(f == "uni"){ # uniform kernel
		y = 0.5 *(abs(x)<1)
	}else if(f == "spline"){
...
IVSurface = function(VarEst,huse,dataday){ #(IVS, gamma2, nu2)

# fix bandwidth
     huse = huse
 
  if (missing("VarEst")){ # just estimate the function
  #  xmin  = c(0.85, 0.047222) # February
  #  xmax  = c(1.15, 0.047222) 
    xmin  = c(0.90, 0.047222)
...
#    data  = data[which(data[,7]>0.89),] # February
#    data  = data[which(data[,7]<1.12),]
  ...
  while (i<=I){
    ...
    sol   = nmgolden(LSKernIVSEst,0.01,0.2,0.85,1e-2,data,h,x)    

 IVS[i,3] = sol$xmin
    cbind(i,sol$xmin)
    if (missing("VarEst")==0){ # if a variance is to be estimated
      data[i,8] = sol$xmin
    }  
    i     = i+1
  }
...
# first part computes  Call option, second the put:
  p = (exp((b-r)*tau) *S *pnorm(y+sigma*sqrt(tau)) - K *exp(-r*tau) *pnorm(y) )*(task==1) + (K*exp(-r*tau) * pnorm(-y) - exp((b-r)*tau)*S* pnorm(-y-sigma*sqrt(tau)) )*(task==0)
  if(any(p<0)){
    p[(p<0)]=0
  }
return(p)
...
# Black and Scholes Vega and Volga functions
BSVegaVolga = function(S, K, r, sigma, tau, task, d){ # proc(vega, volga)
  if (missing(d)){
    d = matrix(0,length(S),1)   
  }
...
# Least Squares Kernel Estimation of Implied Volatility
LSKernIVSEst = function(sigma,data,h,x){ # proc(Q) =
  sigmaB = sigma
...
#dataday = read.table("FebruaryData.dat")
...
hdens = 0.0365
fh    = sker(x=seq(min(dataday[,7]),max(dataday[,7]),length=length(dataday[,7])),y=dataday[,7], h=hdens, N = length(Smile[,1]),f="qua",v=Smile[,1])
...
# confidence bands
  alpha  = 0.05
  sh     = sker(x=seq(min(dataday[,7]),max(dataday[,7]),length=length(dataday[,7])),y=dataday[,7]^2, h=hdens, N = length(Smile[,1]),f="qua",v=Smile[,1])
